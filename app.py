...
with st.sidebar:
    ...
    for label, ok in [
        ("dem.tif", dem_file is not None),
        ("risk.csv", risk_file is not None),
        ("villages", village_file is not None),
        ("roads", road_file is not None),
    ]:
        cls = "status-line on" if ok else "status-line"
        mark = "loaded" if ok else "—"
        st.markdown(f'<div class="{cls}">{label} · {mark}</div>', unsafe_allow_html=True)

if not dem_file or not risk_file:
...
try:
    risk_df, lat_col, lon_col = read_risk(risk_file)
except Exception as e:
    st.error(str(e))
    st.stop()

with rasterio.MemoryFile(dem_file.getvalue()) as mem:
